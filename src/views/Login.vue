<template>
  <div class="login-wrap">
    <div class="canvas-box" ref="canvasBoxRef"></div>
    <div class="login-container">
      <h1 class="title">登录</h1>
      <el-form label-position="left" :model="userInfo" :rules="rules" ref="userInfoRef" label-width="0">
        <el-form-item prop="acc">
          <span>账号:</span>
          <el-input clearable type="text" placeholder="请输入账号"
                    maxlength="8" v-model.trim="userInfo.acc" data-key="a"></el-input>
        </el-form-item>
        <el-form-item prop="pass">
          <span>密码:</span>
          <el-input show-password clearable type="password" v-model="userInfo.pass"
        ...
        <el-form-item prop="code">
          <div style="text-align: left;flex: 1">
            <span>验证码:</span>
            <el-input clearable type="text" v-model.trim="userInfo.code"
                      placeholder="验证码"
                      maxlength="4"
                      STYLE=""></el-input>
          </div>
        ...
        <el-form-item prop="userType">
          <span>用户类型:</span>
          <user-type-selector v-model="userType" show-root/>
        </el-form-item>
      </el-form>

      <el-button type="primary" style="width:100%;" @click="submitForm('userInfo')" :loading="loading">登录</el-button>

      <div class="bottom-box">
        <el-link
            v-if="userType!==LOGIN_CODE_ROOT"
            type="danger"
            @click="$router.push({name:'ForgetPassword'})"

        >
          忘记密码【{{ userType === LOGIN_CODE_READER ? '读者' : '图书管理员' }}】？
        </el-link>
        <span v-else/>
        <el-link type="success" @click="$router.push({name:'Register'})">点击注册【{{ userType === LOGIN_CODE_READER ? '读者' : '图书管理员' }}】</el-link>
      </div>
...
const {LOGIN_CODE_READER} = require("@/api/System");
const {LOGIN_CODE_EMP} = require("@/api/System");
...
watch(userType, () => {
  switch (userType.value) {
    case LOGIN_CODE_READER:
      loginHandler = SystemNetwork.readerLogin
      break;
    case LOGIN_CODE_EMP:
      loginHandler = SystemNetwork.empLogin
      break;
    case LOGIN_CODE_ROOT:
      loginHandler = SystemNetwork.rootLogin
      break;
  }
  userInfo.value = {
    acc: '11111111',
    ...
    if (valid) {
      ...
      loginHandler({
        ...
        success: (data) => {
          ...
          store.dispatch('setUser', {...data.result, userType: userType.value,factor})
          router.push("Home")
        },
        fail(text) {
          loading.value = false;
...
function getCode() {
  SystemNetwork.captcha({
    key: userType.value,
    success: (data) => {
      captchaCode.value = data.result.base64
      userInfo.value.code = data.result.code;
    }
  })
}

const rules = {
  acc: [{required: true, type: 'string', min: 8, max: 8, message: '请输入8位账号', trigger: 'blur'}],
  pass: [{required: true, type: 'string', min: 8, max: 16, message: '请输入8~16位密码', trigger: 'blur'}],
  code: [{required: true, type: 'string', min: 4, max: 4, message: '请输入4位验证码', trigger: 'blur'}]
}

const canvasBoxRef = ref()
nextTick(() => {
  let style = getComputedStyle(canvasBoxRef.value);
  const currentUser = () => {
    switch (userType.value) {
      case LOGIN_CODE_READER:
        return '读者';
  ...
  let documentDot = new DocumentDot({
    ...
    callback: {
      callback(_d) {
        setTimeout(() => {
          _d.emitDot(
              {fontSize: 100, text: '图书\n借阅系统'},
              {fontSize: 100, text: currentUser() + '\n登录'},
          )
        }, 2000)
      },
