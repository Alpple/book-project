<template>
  <div class="login-wrap">
    <div class="login-container" style="position:relative;">
      <!--      <div style="position: absolute;right:1em;top:0.5em;z-index: 1">-->
      <!--        <user-type-selector v-model="userType"/>-->
      <!--      </div>-->
      <div style="position: absolute;right:1em;top:0.5em;z-index: 1">
        <el-tag size="large" type="success">{{ userType === LOGIN_CODE_READER ? '读者' : '图书管理员' }}</el-tag>
      </div>
      <div class="content-box">
        <div class="tag">
          <h1 class="title">
            忘记密码了...
          </h1>
          <el-steps direction="vertical" :active="active" finish-status="success">
            <el-step title="手机号码" description="输入你注册账号时用的手机号码"/>
            <el-step title="验证码" description="输入你收到的验证码"/>
            <el-step title="修改密码" description="然后你就可以重设你的密码了"/>
            <el-step title="返回登录" description="最后你可以尝试重新登录"/>
          </el-steps>
          <el-link type="primary" @click="$router.push({name:'Login'})" style="margin-top: .5em">想起密码了.直接前往登录</el-link>
        </div>
        <div class="content">
          <div class="pane" :class="active===0?'active':''">
            <div class="tel-pane default-layout">
              <el-input clearable type="text" placeholder="请输入手机号"
                        maxlength="11"
                        v-model.trim="tel" @input="telChange"></el-input>
              <br>
              <el-button type="primary" @click="telSubmit" :loading="status.tel">
                下一步
              </el-button>
            </div>
          </div>
          <div class="pane" :class="active===1?'active':''">
            <div class="v-code-pane default-layout">
              <h4>验证码已发送！注意查收！</h4>
              <div style="display: flex">
                <el-input clearable type="text" placeholder="请输入4位验证码"
                          maxlength="4"
                          v-model="code"
                          style="margin-right: 1em">
                </el-input>
                <el-button type="success" @click="reCodeSubmit" :disabled="status.reCode">{{ codeText }}</el-button>
              </div>
              <br>
              <el-button type="primary" @click="verificationCode" :loading="status.code">
                下一步
              </el-button>
            </div>
          </div>
          <div class="pane" :class="active===2?'active':''">
            <div class="pass-pane default-layout">
              <el-form label-position="left" :model="passInfo" :rules="rules" ref="passInfo" label-width="0">
                <div>这里显示用户名</div>
                <el-form-item prop="password">
                  <span>密码:</span>
                  <el-input show-password clearable type="password" v-model="passInfo.password" auto-complete="off"
                            placeholder="请输入8~16位密码" maxlength="16" @input="passwordInput"></el-input>
                  <div class="check-list" :hidden="passInfo.password.length===0">
                    <span class="item">
                       <span>
                            <span :hidden="!status.passwordHasNumber">
                              <el-icon size="1em" color="#67C23A"><circle-check/></el-icon>
                            </span>
                            <span :hidden="status.passwordHasNumber">
                               <el-icon size="1em" color="#F56C6C"><circle-close/></el-icon>
                            </span>
                       </span>
                      <span>数字</span>
                     </span>
                    <span class="item">
                       <span>
                          <span :hidden="!status.passwordHasWord">
                            <el-icon size="1em" color="#67C23A"><circle-check/></el-icon>
                          </span>
                          <span :hidden="status.passwordHasWord">
                             <el-icon size="1em" color="#F56C6C"><circle-close/></el-icon>
                          </span>
                       </span>
                      <span>字母</span>
                    </span>
                    <span class="item">
                      <span>
                        <span :hidden="!status.passwordHasOther">
                          <el-icon size="1em" color="#67C23A"><circle-check/></el-icon>
                        </span>
                        <span :hidden="status.passwordHasOther">
                           <el-icon size="1em" color="#F56C6C"><circle-close/></el-icon>
                        </span>
                      </span>
                      <span>字符</span>
                    </span>
                  </div>
                </el-form-item>
                <el-form-item prop="rePassword">
                  <span>确认密码:</span>
                  <el-input show-password clearable type="rePassword" v-model="passInfo.rePassword"
                            placeholder="请输入确认密码" maxlength="16"></el-input>
                </el-form-item>
              </el-form>
              <el-button type="primary" @click="submitForm('passInfo')" :loading="status.password">确认修改</el-button>
            </div>
          </div>
          <div class="pane" :class="active===3?'active':''">
            <div class="pass-pane default-layout">
              <h4>修改密码成功，请务必刻入DNA.</h4>
              <el-button type="primary" @click="$router.push('Login')">点击返回登录</el-button>
            </div>
          </div>
        </div>
      </div>

    </div>
  </div>
</template>
<script setup>
import {LOGIN_CODE_READER} from "@/api/System";
</script>
<script>
import {LOGIN_CODE_READER, SystemNetwork} from "@/api/System";
import UserTypeSelector from "@/components/user-type-selector";

const spaceReg = /\s/g
// 数字
const numberReg = /[a-zA-Z]/g
// 字母
const wordReg = /[0-9]/g
// 特殊字符： ;'/.,~!@#!$#$^&(等
const otherReg = /((?=[\x21-\x7e]+)[^A-Za-z0-9])/g


export default {
  name: 'ForgetPassword',
  components: {UserTypeSelector},
  data() {
    const validatePassword = (rule, value, callback) => {
      // 包含空白字符
      if (spaceReg[Symbol.search](value) !== -1) {
        callback(new Error('密码不能包含空白字符'))
      }

      if (this.passwordInput(value) < 2) {
        callback(new Error('密码必须包含：字母、数字、符号任意两种'))
      }
      callback()
    }
    const validateRePassword = (rule, value, callback) => {
      if (this.passInfo.password !== this.passInfo.rePassword) {
        callback("两次输入的密码不一致")
      }
      callback()
    }
    return {
      userType: localStorage.getItem("user-type"),
      codeText: '重新发送',
      tel: '11111111111',
      code: '',
      passInfo: {
        password: 'a1111111',
        rePassword: 'a1111111'
      },
      status: {
        passwordHasNumber: false,
        passwordHasWord: false,
        passwordHasOther: false,
        tel: false,
        code: false,
        codeTime: 0,
        reCode: true,
        password: false,
      },
      active: 0,
      //rules前端验证
      rules: {
        password: [
          {required: true, type: 'string', min: 8, max: 16, message: '请输入8~16位密码', trigger: 'blur'},
          {validator: validatePassword, trigger: 'blur'}
        ],
        rePassword: [
          {validator: validateRePassword, trigger: 'blur'}
        ],
      }
    }
  },
  methods: {
    telSubmit() {
      if (this.tel.length !== 11) {
        this.$notify.error("请输入正确的手机号！")
        return;
      }
      let that = this;
      this.status.tel = true
      SystemNetwork.verTel({
        tel: this.tel,
        success() {
          that.status.tel = false
          that.active = 1
          // that.status.code = fa;
          that.getTelCode(() => {
          })
        },
        fail() {
          that.status.tel = false
        },
        reject() {
          that.status.tel = false
        }
      })
    },
    reCodeSubmit() {
      let that = this;
      this.getTelCode(function () {
      })
    },
    verificationCode() {
      if (this.code.length !== 4) {
        this.$notify.error("请输入正确的验证码！")
        return;
      }
      let that = this;
      that.status.code = true
      SystemNetwork.codeChek({
        code: that.code,
        key: this.tel,
        success() {
          that.status.code = false
          that.active = 2
        },
        fail() {
          that.status.code = false
        },
        reject() {
          that.status.code = false
        }
      })
    },
    //表单提交
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.modifyPass_()
        }
      })
    },
    modifyPass_() {
      //  修改成功
      let that = this;
      that.status.password = true
      let modifiy = this.userType === LOGIN_CODE_READER
          ? SystemNetwork.modifyReaderPassword
          : SystemNetwork.modifyEmpPassword;
      modifiy({
        password: that.passInfo.password,
        tel: that.tel,
        success() {
          that.$notify.success("修改密码成功！")
          that.status.password = false
          that.active = 3
        },
        fail() {
          that.status.password = false
        },
        reject() {
          that.status.password = false
        }
      })
    },
    getTelCode(callback = () => undefined) {
      let that = this;
      that.status.reCode = true
      that.status.code = false;
      that.status.codeTime = 0;
      clearInterval(intervalG)
      intervalG = setInterval(function () {
        if (that.status.codeTime++ > 5) {
          clearInterval(intervalG)
          that.status.reCode = false
          that.codeText = '验证码'
          that.status.codeTime = 0;
          return;
        }
        that.codeText = (5 - that.status.codeTime + 1) + '秒'
      }, 1000)
      SystemNetwork.telCode({
        tel: this.tel,
        success(data) {
          that.$notify.success("获取验证码成功");
          that.code = data.result.code;
          callback();
        }
      })
    },
    // 输入时，限制电话只能输入数字
    telChange(value) {
      this.tel = value.replace(/\D/g, '')
    },
    passwordInput(value) {
      let count = 0;
      if (numberReg[Symbol.search](value) !== -1) {
        count++
        this.status.passwordHasNumber = true;
      } else {
        this.status.passwordHasNumber = false;
      }

      if (wordReg[Symbol.search](value) !== -1) {
        count++
        this.status.passwordHasWord = true;
      } else {
        this.status.passwordHasWord = false;
      }

      if (otherReg[Symbol.search](value) !== -1) {
        count++
        this.status.passwordHasOther = true;
      } else {
        this.status.passwordHasOther = false;
      }
      return count
    },
  }
}
var intervalG;
</script>

<style scoped lang="less">
.login-wrap {
  box-sizing: border-box;
  display: flex;
  justify-content: center;
  align-items: center;
  width: 100%;
  height: 100vh;
  background-image: url(data:image/svg+xml;base64,);
  background-color: transparent;;
  background-repeat: no-repeat;
  background-position: center right;
  background-size: 100%;
}

.login-container {
  border-radius: 1rem;
  margin: 1rem auto;
  width: 550px;
  padding: 30px 45px;
  background: rgba(255, 255, 255, 0.52);
  box-shadow: 0 0 20px 2px rgba(0, 0, 0, 0.1);
}

.title {
  text-align: left;
  color: #505458;
  margin: 0 0 .5em 0;
}

.bottom-box {
  display: flex;
  width: 100%;
  justify-content: space-between;
  margin-top: .5em;
}

.check-list {

  .item {
    display: inline-flex;

    & > span:first-child {
      display: inline-block;
      margin-left: .5em;
      margin-right: 2px;
    }
  ;
  }
}

.content-box {
  display: flex;
  height: 300px;

  .content {
    flex: 1;
    margin: 1em;
    //background: #42b983;
    position: relative;
    overflow: hidden;

    .pane {
      background: white;
      display: block;
      width: 100%;
      height: 100%;
      overflow: hidden;
      position: absolute;
      top: -100%;
      transition: top 1s;

      &.active {
        top: 0;
      }

      .default-layout {
        display: flex;
        height: 100%;
        justify-content: center;
        align-items: center;
        flex-direction: column;
      }
    }
  }

  .tag {
    display: flex;
    flex-direction: column;
    margin-right: 1em;
  }
}

.tel-pane {

  & > form {
    width: 100%
  }
}


</style>
