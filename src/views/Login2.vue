    ...
    <div class="login-container">
      ...
      <el-form label-position="left" :model="userInfo" :rules="rules" ref="userInfoRef" label-width="0">
        ...
        <el-form-item prop="pass">
          <span>密码:</span>
          <el-input show-pass clearable type="pass" v-model="userInfo.pass"
      ...
      <div class="bottom-box">
        ...
        <span v-else/>
        <el-link type="success" @click="$router.push({name:'Register'})">点击注册</el-link>
      </div>
  ...
  userInfoRef.value.validate((valid) => {
    if (valid) {
      ...
      loginHandler({
        ...
        success: (data) => {
          loading.value = false;
          store.dispatch('setUser', {...data.result, userType: userType.value})
          router.push("Home")
        },
        fail(text) {
          loading.value = false;
