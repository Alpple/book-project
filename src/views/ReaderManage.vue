  ...
  <el-table :data="tableData" stripe style="width: 100%;margin:1em 0">
    ...
    <el-table-column label="密 码">
      <template #default="{row,$index}">
        <el-button size="small" plain type="danger" @click="resetPassword(row)" :loading="row.resetBtnLoading">重置密码</el-button>
      </template>
    </el-table-column>
...
const resetPassword = (row) => {
  row.resetBtnLoading= true;
  const func = () => {
    row.resetBtnLoading = false;
  }
  UserNetwork.resetReaderPassword({
    id: row.id,
    success: () => {
...
watch([currentPage, pageSize], () => {
  router.replace(`/home/reader/manage/${currentPage.value}/${pageSize.value}`)
})
